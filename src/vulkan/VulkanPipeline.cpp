...
void VulkanPipeline::createPipeline(PipelineType type, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline) {
	...
	rasterizer.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE;	// Matches OpenGL models.
	...
	// Pipeline layout - both pipelines use the same descriptor set layouts for compatibility.
...
std::pair<std::vector<uint32_t>, std::vector<uint32_t>> VulkanPipeline::loadShaders(PipelineType type) {
	...
	return {vertShaderCode, fragShaderCode};
}
...
std::vector<char> VulkanPipeline::readFile(const std::string& filename) {	//TJ: Again, does file loading really seem
	std::ifstream file(filename, std::ios::ate | std::ios::binary);			//	appropriate amidst all the Pipeline minutiae?
...
std::pair<std::vector<uint32_t>, std::vector<uint32_t>> VulkanPipeline::cannedShaders() {
	// Embedded SPIR-V bytecode for untextured shaders.
	// These are fallback shaders compiled from basic vertex and fragment shader sources.

	// Basic vertex shader - Input: position (vec3), normal (vec3), color (vec3), Output: gl_Position, fragColor
	std::vector<uint32_t> vertShaderCode = {
	...
	// Basic fragment shader - Input: fragColor (vec3), Output: outColor (vec4)
	std::vector<uint32_t> fragShaderCode = {
